...
- Observability (request-id, structured logs + future cross-cutting concerns): `docs/development/observability.md` (`Nene\Xion\RequestId` + `X-Request-ID` header + Monolog `extra.request_id`; `NENE_LOG_FORMAT=json` for log aggregators; recipe for future per-request decorations)
