...
- Observability (request-id, structured logs, Server-Timing + future cross-cutting concerns): `docs/development/observability.md` (`Nene\Xion\RequestId` + `X-Request-ID` header + Monolog `extra.request_id`; `NENE_LOG_FORMAT=json`; `NENE_SERVER_TIMING_ENABLED=1` for `Server-Timing: app;dur=X.X`; recipe for future per-request decorations)
