...
- [ ] **Multipart upload operations** use `requestBody.content['multipart/form-data']` with a `type: object` schema declaring each form field (use `type: string, format: binary` for the file). The contract test probes such operations with an **empty body** (it only reads `application/json` examples) — make sure the documented `responses:` list includes whatever status the controller returns when the multipart `file` part is missing (typically `400` `UPLOAD-FILE-REQUIRED`). Operations that cannot tolerate the empty-body probe (e.g. destructive writes) must set `x-nene-runtime-probe: skip`.
